...
import { z } from "zod";
import { plantFormSchema } from "../domain/entities/PlantNote";
...
export function PlantForm({ noteId, orderId }: PlantFormProps) {
    const { setNewNote, notes } = usePlantOSStore()
    const { orders } = useOSStore()
    const [showDatePicker, setShowDatePicker] = useState(false);
    const order = orders[orderId] as OSPlantDTO
    const note = noteId ? notes[noteId] : null

    type NoteFormData = z.infer<typeof plantFormSchema>;
